    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card-footer">
                ...
                <div class="form-group">
                    ...
                    @if(isset($author) && $author->slug)
                        <input type="text" class="form-control" name="slug" id="slug"
                               value="{{ old('slug', $author->slug) }}" readonly>
                        <input type="hidden" name="original_slug" value="{{ $author->slug }}">
                    @else
                        <input type="text" class="form-control" name="slug" id="slug"
                ...
                <div class="form-group">
                    <label for="content">Descripción del autor</label>
                    <textarea class="form-control w-100" name="content" id="content"
                              cols="30" rows="2" placeholder="Descripción del autor">{{ old('content', $author->content ?? '') }}</textarea>
                    @error('content')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="row">
                    <div class="col-sm-6">
                        ...
                        @if(isset($author) && $author->img)
                            <div class="mb-2">

                                <img loading="lazy" src="{{ asset('images/user/author/' . $author->img) }}"  style="filter: grayscale(0%); width: 30%!important;"
                                alt="{{ $author -> name }}">
                    ...
                    <div class="col-sm-6">
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="statusx" id="status_published"
                                   value="PUBLISHED" {{ old('statusx', $author->statusx ?? 'PUBLISHED') == 'PUBLISHED' ? 'checked' : '' }}>
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="statusx" id="status_draft"
                                   value="DRAFT" {{ old('statusx', $author->statusx ?? '') == 'DRAFT' ? 'checked' : '' }}>
